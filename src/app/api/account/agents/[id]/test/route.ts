...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({
    ...
    selection: {
      ...
      note: `The test job is a real POST /api/v1/requests (execution_mode: push) as your user wallet. Category ${readiness.fixture.category} is taken from this agent’s specialties (first listed, or a POST body override) so invite_agent_ids does not get dropped as specialty_mismatch. Without invite_agent_ids the marketplace still invites Top-K hireable html_to_pdf agents.`,
    },
  });
}
