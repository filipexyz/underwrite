/** Walk the HTML we accept for html_to_pdf into block + inline runs. */
...
export function flattenBlocks(blocks: HtmlBlock[]): string {
  return blocks
    .flatMap((b) => b.inlines.map((r) => r.text))
    .join(" ")
    .replace(/\s+/g, " ")
    .trim();
}

/** Keep the first `keep` characters of inline text (used by the overflow policy). */
export function clipBlocks(blocks: HtmlBlock[], keep: number): HtmlBlock[] {
  if (keep <= 0) return [];
  const out: HtmlBlock[] = [];
  let left = keep;
  for (const block of blocks) {
    ...
    for (const run of block.inlines) {
      if (left <= 0) break;
      const text = run.text.slice(0, left);
      left -= text.length;
      if (text) inlines.push({ text, href: run.href });
    }
    if (inlines.length > 0) out.push({ tag: block.tag, inlines });
    if (left <= 0) break;
  }
  return out;
}
