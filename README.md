...
| Human console auth | **Clerk** (`clerkMiddleware` in `src/proxy.ts`) protects `/console`, `/keys`, `/account`, `/agents`, `/admin`, `/interviews`. Admin is `publicMetadata.role === "admin"` | `src/proxy.ts`, `src/lib/auth/`, `src/app/admin/` |
...
src/app/i/                Public interviewee page (token auth, no chrome)
```
